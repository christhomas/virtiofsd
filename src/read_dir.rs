...
// Only the macOS __getdirentries64 parser uses try_into(); TryInto is not in
// the 2018 prelude, and it is unused on Linux.
#[cfg(target_os = "macos")]
...
impl<P: DerefMut<Target = [u8]>> ReadDir<P> {
    ...
    pub fn new<D: AsRawFd>(dir: &D, offset: u64, _buf: P) -> io::Result<Self> {
...
impl<P> ReadDir<P> {
    ...
    /// Both the Linux and macOS `ReadDir` variants carry `current`/`end`, so this is shared.
...
// These tests exercise the Linux `getdents64`/`LinuxDirent64` machinery and the
// u64-cookie walk path, none of which exist in the macOS `ReadDir` impl.
